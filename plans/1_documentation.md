# Plan: Documentation Strategy for Aionoscope

## Goal
Establish a documentation standard that ensures Aionoscope is accessible to human researchers while being unambiguous and easy to navigate for AI coding agents.
...
- **Self-Contained Examples**: `examples/` should contain runnable scripts that demonstrate correct usage patterns. Agents can read these to understand how to compose modules.
...
Create an `examples/` directory with minimal scripts:
...
Create a `aiono/CONTEXT.md` (or similar) that concatenates the public interfaces of `core`, `processes`, and `views`. This allows an agent to quickly ingest the entire library surface area in one read.
