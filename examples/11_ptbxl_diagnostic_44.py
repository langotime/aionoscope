...
def main() -> None:
    ...
    diagnostic_codes = ptbxl_codes_by_group("diagnostic")
    sampler = GroupCycleSampler(
        scp_codes=scp_codes,
        target_codes=diagnostic_codes,
    ...
    rng.manual_seed(2028)
    pipeline = SynthPipeline(process=process, views=views).to(device)
    batch = pipeline(batch_size=len(diagnostic_codes), device=device, rng=rng)

    x = batch["clean"].x.detach().cpu()  # [B, C, L]
    y = batch["clean"].y["scp"].detach().cpu()  # [B, S]
    diag_indices = batch["clean"].meta["process"]["label_groups"]["diagnostic"]
    ...
    fig.suptitle("ECGProcess: PTB-XL diagnostic codes (lead II + V2 + V5)")

    for code_idx, code in enumerate(diagnostic_codes):
        col = code_idx % num_cols
        row = code_idx // num_cols
        ax = axes[row][col]

        matches = torch.nonzero(y[:, diag_indices[code_idx]], as_tuple=False).flatten()
        if matches.numel() == 0:
            raise ValueError(f"No samples for diagnostic code '{code}'.")
    ...
    out_path = output_dir / "ptbxl_diagnostic_44.png"
    fig.tight_layout()
